...
export default async function ProfileConfigPage({
  ...
  const tenantId = params.tenant_id || ctx.actor.tenantId;
  ...
  if (tenantId && ctx.actor.userId) {
    // Tenant user
    resolvedConfig = await configResolver.resolveTenantUserConfig(
      tenantId,
      ctx.actor.userId
    );
    personalConfig = await configRepo.getTenantUserPersonalConfig(
      tenantId,
      ctx.actor.userId
    );
  } else if (vendorId && ctx.actor.userId) {
    // Vendor user
    resolvedConfig = await configResolver.resolveVendorUserConfig(
      vendorId,
      ctx.actor.userId
    );
    personalConfig = await configRepo.getVendorUserPersonalConfig(
      vendorId,
      ctx.actor.userId
    );
  }
    ...
    <div className="na-container na-mx-auto na-p-6">
      <div className="na-mb-8">
        ...
        <p className="na-body na-text-muted">
          Configure your personal preferences. These override all other
          ...
          <div className="na-card na-p-6">
            ...
            <div className="na-text-sm na-text-muted">
              ...
              <ul className="na-list-disc na-list-inside na-space-y-1">
                ...
                {resolvedConfig.sources.tenant_user_admin && (
                  <li>Tenant User Admin</li>
                )}
                {resolvedConfig.sources.tenant_user_personal && (
                  <li>Your Personal</li>
                )}
                {resolvedConfig.sources.vendor_global && <li>Vendor Global</li>}
                {resolvedConfig.sources.vendor_user_personal && (
                  <li>Your Personal</li>
                )}
              </ul>
            </div>
      ...
      {!resolvedConfig && (
        <div className="na-card na-p-6">
          <p className="na-body">
            Please select a tenant or vendor to view configuration.
          </p>
        </div>
      )}
    </div>
