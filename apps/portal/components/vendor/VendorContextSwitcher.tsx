...
import {
  ...
  type VendorGroupAccess,
} from "@/src/repositories/vendor-group-repository";
import { useEffect, useState } from "react";

interface VendorContextSwitcherProps {
  currentTenantId: string | null;
  onTenantChange?: (tenantId: string | null) => void; // Optional - uses URL navigation if not provided
  userId: string;
}

export function VendorContextSwitcher({
  currentTenantId,
  onTenantChange,
  userId,
}: VendorContextSwitcherProps) {
  const [accessibleSubsidiaries, setAccessibleSubsidiaries] = useState<
    VendorGroupAccess[]
  >([]);
  ...
  useEffect(() => {
    const loadSubsidiaries = async () => {
      try {
        ...
        const subsidiaries = await vendorGroupRepo.getAccessibleSubsidiaries(
          userId
        );
        ...
        if (tenantIds.length > 0) {
          const response = await fetch(
            `/api/tenants?ids=${tenantIds.join(",")}`
          );
    ...
    <div className="na-card na-p-4 na-flex na-items-center na-gap-4">
      ...
      <select
        ...
        onChange={(e) => {
          ...
          const newTenantId = value === "all" ? null : value;
          if (onTenantChange) {
            onTenantChange(newTenantId);
          } else {
            // URL-based navigation for Server Component pages
            const url = new URL(window.location.href);
            if (newTenantId) {
              url.searchParams.set("tenant_id", newTenantId);
            } else {
